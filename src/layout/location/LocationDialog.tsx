  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        <div
          className="items-center flex flex-grow hover:bg-pink-100/40 rounded-xl"
        ...
        >
          <Button variant="ghost" size={"icon"} className="hover:scale-105 ">
            <MapPin
              size={20}
              className={`${
                dashboard ? "text-black dark:text-white" : "text-white"
              }`}
            />
          </Button>
          {city || country ? (
            <input
              ...
              className={` ${
                dashboard ? "text-black dark:text-white" : "text-white"
              } text-sm w-full rounded p-2 bg-transparent`}
              value={`${city || ""}, ${country || ""}`}
              readOnly
            />
          ) : (
              ...
              <DialogFooter className="my-4 mx-4">
                {!chooseField ? (
                  ...
                  <div className="flex gap-4">
                    ...
                    <Button onClick={() => setManualField(true)}>
                      Choose Manually
                    </Button>
                  </div>
                )}
