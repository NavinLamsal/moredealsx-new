...
const BasicInfoForm = ({ businessData }: { businessData: any }) => {
    ...
    const [errors, setErrors] = useState<{ BusinessName?: string; BusinessRegistration?: string; BusinessEmail?: string; BusinessPhone?: string, BusinessLocation?: string }>({});
    ...
    const handlePlaceSelected = async (coordinates: {
        ...
        setFormData((prev) => {
            const updatedData = {
                ...prev, BusinessLocation: placeName,  // Corrected typo
    ...
    const validate = async (fieldValues = formData) => {
        ...
        if ("BusinessName" in fieldValues) {
            tempErrors.BusinessName = validateRequired(fieldValues.BusinessName || "", "Business Name");
        }


        if ("BusinessRegistration" in fieldValues) {
            tempErrors.BusinessRegistration = validateRequired(fieldValues.BusinessRegistration || "", "Business Registration");
        }

        if ("BusinessPhone" in fieldValues) {
            if (fieldValues.BusinessPhone !== "") {
                tempErrors.BusinessPhone = validatePhoneNumber(fieldValues.BusinessPhone || "");
            }
        }

        if ("BusinessLocation" in fieldValues) {
            tempErrors.BusinessLocation = validateRequired(fieldValues.BusinessLocation || "", "Business Location");
        }
