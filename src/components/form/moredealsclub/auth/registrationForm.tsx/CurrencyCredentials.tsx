...
const CurrencyForm = () => {
  ...
  const validate = async (
    ...
    if ("country" in fieldValues) {
      tempErrors.country = validateRequired(
        fieldValues.country || "",
        "Country"
      );
    }
    ...
    if ("agreeToTerms" in fieldValues) {
      tempErrors.agreeToTerms = fieldValues.agreeToTerms
        ? ""
      ...
      if (data.success) {
        if (registerMethod === "EMAIL") {
          localStorage.setItem(
            "otp_username",
            JSON.stringify({ email, via: "email" })
          );
        } else if (registerMethod === "PHONE") {
          localStorage.setItem(
            "otp_username",
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4 p-2">
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            className={`h-5 w-5 border rounded focus:ring-2 focus:ring-primary ${
              errors.agreeToTerms
          ...
          >
            ...
            <a
              href="#"
              className={`  underline ${
                errors.agreeToTerms ? "text-orange-500" : "text-primary"
              }`}
            >
