...
const ChangePasswordInsideForm = () => {
    ...
    const validate = async (fieldValues = formData) => {
        ...
        if ("oldPassword" in fieldValues) {
            tempErrors.oldPassword = validateRequired(fieldValues.oldPassword || "", "Old Password")
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const errorMessage = error?.response?.errors?.non_field_errors?.[0] || error?.response?.data?.message || "Something went wrong, please try again";
