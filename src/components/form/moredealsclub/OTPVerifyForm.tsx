...
const OTPVerifyForm: React.FC = () => {
  const { userType, } = useSelector((state: RootState) => state.registration);
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      const storedUsername = localStorage.getItem("otp_username");
      if (storedUsername) {
  ...
  return (
    <form onSubmit={handleSubmit} className="p-4 space-y-4">
      ...
      <p className="text-center text-sm text-muted-foreground">
        ...
        {formData.via === "email" ? formData.email : formData.phone} to verify
        your account.
      </p>
