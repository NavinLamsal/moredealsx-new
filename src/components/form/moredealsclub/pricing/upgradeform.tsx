...
const UpgradeForm = ({ userType, membershiptype }: { userType: "BUSINESS" | "NORMAL", membershiptype: any }) => {
    ...
    const availablePackages = packages?.[formData.plan_type]?.[formData.plan_time] || [];
    ...
    useEffect(() => {
        if (!formData.package && membershiptype && availablePackages.length > 0) {
            const pkg = availablePackages.find(p => p.name === membershiptype.membership_name);
            if(pkg){
                setFormData(prev => ({
                    ...prev,
                    package: pkg.id,
    ...
    const steps = [
        {
            component: Step1PopForm,
            props: { data: formData, errors, serverError, setData, onNext: () => handleNext(1), isLoading },
        },
        {
            component: Step2Form,
            props: { purchasingInformation: purchasingInfo, data: formData, errors, serverError, setData, onNext: () => handleNext(2), onBack: () => setStep(0), isLoading },
...
const TermsAndAlerts = ({ plan_time }: { plan_time: "monthly" | "yearly" }) => {
    const endDate = useMemo(() => {
        const date = new Date();
        plan_time === "monthly" ? date.setMonth(date.getMonth() + 1) : date.setFullYear(date.getFullYear() + 1);
    ...
    return (
        <div className="mt-6 p-4 xl:pt-0 rounded-md text-sm">
            ...
            <p className="mt-2 text-muted-foreground text-sm">Your plan will be valid until <strong>{endDate}</strong>.</p>
        </div>
    );
};
