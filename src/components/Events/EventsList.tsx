...
const EventList = () => {
  ...
  const {
    ...
    queryKey: ["Event list"],
    queryFn: ({ pageParam = 1 }) => fetchEventsList(pageParam),
  ...
  return (
    <div className="">
        ...
        {data?.pages.map((page, pageIndex) =>
          page.data.map((event, index) => (
            <div key={`${pageIndex}-${index}`}>
              <div className="flex-shrink-0 w-60 lg:w-72" key={event.id}>
      ...
      {isFetchingNextPage && (
        <p className="text-center mt-4 text-gray-600">Loading more events...</p>
      )}
    </div>
  );
};
