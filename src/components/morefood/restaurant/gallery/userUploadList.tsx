...
const UserUploadList = ({ slug }: { slug: string }) => {
  const { fetchRestaurantUserGallery } = useFetchRestaurant();
  ...
  const {
    ...
    queryKey: ["user-uploads", slug],
    queryFn: ({ pageParam = 1 }) => fetchRestaurantUserGallery(slug, pageParam, 20), // Fetch 6 images per page
