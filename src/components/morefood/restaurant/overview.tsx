...
import { Eye, EyeOffIcon, GlobeIcon, Mail, Phone } from 'lucide-react'
...
const Overview = ({ details }: { details: Restaurant }) => {
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 gap-2 container px-2  mx-0  ">

        <div className="flex items-center gap-2 ">
          {details?.website_link && (
            <a href={details?.website_link} target="_blank">
              <Button
                variant={'link'}
                className="bg-transparent py-6 px-2 text-foreground"
              >
                <GlobeIcon size={24} /> &nbsp;Website
              </Button>
            </a>
          )}
        ...
        <div className="grid gap-3 mb-4 px-2">
          <div className="flex gap-4 items-center text-sm">
            ...
            <p className="cursor-pointer" onClick={() => setShowEmail(!showEmail)}>
              {showEmail ? <EyeOffIcon  size={16} /> : <Eye size={16} />}
            </p>
          </div>
          ...
          <div className="flex gap-2 text-sm">
            <Phone size={16} />
            {showPhone ? (
              <a href={`tel:${details.contact_no}`}>
                <span className="ml-2 md:mx-2">{details.contact_no}</span>
              </a>
            ) : (
              "XXXXXXXXXXX"
            )}
            <p className="cursor-pointer" onClick={() => setShowPhone(!showPhone)}>
              {showPhone ? <EyeOffIcon  size={16} /> : <Eye  size={16}/>}
            </p>
          </div>
          
          <DescriptionSection description={details.long_description} name={details.name}/>
