...
export async function generateMetadata({params }:{ params: Promise<{ slug: string }>}): Promise<Metadata> {
  ...
  return {
    ...
    description: details.short_description || `Explore delicious food at ${details.name}.`,
    openGraph: {
      title: `${details.name} | MoreFood`,
      description: details.short_description || `Explore delicious food at ${details.name}.`,
      url: `https://yourwebsite.com/restaurant/${slug}`,
      images: [{ url: details.banner || details.logo ||'/default-restaurant.jpg', width: 1200, height: 630, alt: details.name }],
      type: 'website',
    },
    twitter: {
...
// 🔹 Layout Component
const RestaurantLayout = async ({ children, params }: { children: React.ReactNode;  params: Promise<{ slug: string }>} ) => {
