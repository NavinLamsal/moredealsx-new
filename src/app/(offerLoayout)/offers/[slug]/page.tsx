...
//   try {
//     const blogs: OfferType = await getOfferDetails(slug);
//     const previousImages = (await parent).openGraph?.images || [];

//     return {
//       title: blogs.name,
//       description: blogs.description,
//       keywords: blogs.description,
//       robots: "index, follow",
//       openGraph: {
//         url: canonicalUrl,
//         title: blogs.name,
...
async function BlogDetail({ slug }: { slug: string }) {
  try {
    ...
    // console.log("Offer Detail: ", blogs);

    const discount = calculateDiscount(blogs);

    const { statusText, badgeColor } = getOfferStatus(
      blogs.start_date,
      blogs.end_date
    );
    return (
      <div className="max-w-5xl mx-auto">
        ...
        <div className="px-4 lg:px-0">
          ...
          <p className="text-gray-600 dark:text-gray-300 mt-2">{statusText}</p>

          <div className="prose max-w-full prose-headings:text-card-foreground text-card-foreground prose-strong:text-card-foreground text-justify ">
          ...
          {/* {blogs?.discount_amount !== null &&
            Number(blogs?.discount_amount) > 0 && (
              <>
                <span className="text-xs leading-tight">
                  Discount {blogs?.currency_symbol}
                  {blogs?.discount_amount} off
                </span>
              </>
            )} */}

          <div className="prose max-w-full prose-headings:text-card-foreground text-card-foreground prose-strong:text-card-foreground text-justify mt-4 ">
            <p>{blogs.description}</p>
          </div>
          {/* <div
          ...
          {/* {blogs.url && blogs.url !== "" && (
            <div>
              <p className="font-semibold text-gray-700 dark:text-gray-300">
                ...
                Visit{" "}
                <a href={blogs.url}>
                  <Button variant="link">{blogs.url}</Button>
                </a>
              </p>
            </div>
          )} */}
          {/* <p className="text-gray-600 dark:text-gray-300 mt-2">
            Published on:{" "}
            {moment
              .utc(blogs.created_at)
              .local()
              .format("MMMM DD, YYYY h:mm A")}
          </p>
          {blogs.updated_at && blogs.created_at !== blogs.updated_at && (
            <p className="text-gray-600 dark:text-gray-300 mt-2">
              Updated on:{" "}
              {moment
                .utc(blogs.updated_at)
                .local()
                .format("MMMM DD, YYYY h:mm A")}
            </p>
          )} */}
    ...
    return (
      <div className="text-center py-10">
        ...
        <p className="text-gray-500">
          The requested offer could not be retrieved.
        </p>
      </div>
    );
  }
