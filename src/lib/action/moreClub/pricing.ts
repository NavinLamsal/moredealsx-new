...
export const fetchPackages = createAsyncThunk(
  ...
  async ({ type, cycle }: { type: "BUSINESS" | "NORMAL"; cycle: "monthly" | "yearly" }, { dispatch, getState }) => {
    ...
    try {
      const response = await MoreClubApiClient.get(`subscriptions/list/?plan_type=${type}&plan_time=${cycle}`);
